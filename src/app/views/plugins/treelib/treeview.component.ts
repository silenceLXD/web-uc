import {Component, Input, Output, EventEmitter, SimpleChanges, OnChanges, TemplateRef, ViewChild, ElementRef} from '@angular/core';
...
export class TreeviewComponent implements OnChanges {
  ...
  @ViewChild('inputItem') inputItemRef: ElementRef;

  constructor(public i18n: TreeviewI18n,
              private defaultConfig: TreeviewConfig,
              private eventParser: TreeviewEventParser) {
    this.config = this.defaultConfig;
    this.allItem = new TreeviewItem({name: 'All', userId: undefined});
  ...
  private filterItem(item: TreeviewItem, filterText: string): TreeviewItem {
    const isMatch = _.includes(item.name.toLowerCase(), filterText);
    if (isMatch && _.isNil(item.children)) {
      return item;
    } else {
